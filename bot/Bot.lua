...
function create_config( )
	...
	sudo_users = {472688602 ,SUDO,BotMaTaDoR_id,BotMaTaDoR_idapi},
	admins = {},
	disabled_channels = {},
	...
	if data.sender_user_id then
		if cmd == "id" then
			local function id_cb(arg, data)
				...
				text = M_START.."*اسم المستخدم :* @"..check_markdown(data.username).."\n"..M_START.."*الاسم :* "..user_name.."\n"..M_START.."*الايدي :* `"..data.id.."`"
...
function msg_valid(msg)
	...
	if (matches == "bot" ) or (matches == "بوت" ) or (matches == "امينو" ) then
		if redis:get(RedisIndex.."lock_cmd"..msg.chat_id) and not is_mod(msg) then return else
			tdbot.sendMention(msg.chat_id,msg.sender_user_id, msg.id,M_START..'انا شغال لا تهتم 🙄'..EndPm,7, tonumber(Slen("ping")))
		end
	end
	if (matches == "id"  or matches == "ايدي" or matches == "إيدي") and tonumber(msg.reply_to_message_id) == 0 then
		if redis:get(RedisIndex.."lock_cmd"..msg.chat_id) and not is_mod(msg) then return else
		...
		local gap_info_msgs = tonumber(redis:get(RedisIndex..'msgs:'..msg.chat_id) or 0)
			if not data.photos[0] then
				...
				tdbot.sendPhoto(msg.chat_id, msg.id, data.photos[0].sizes[1].photo.persistent_id, 0, {}, 0, 0, ''..M_START..'رسائل المجموعه  : '..msg.chat_id..'\n'..M_START..'رسائلك  : '..msg.sender_user_id..'\n'..M_START..'عدد رسائل المجموعه : [ '..gap_info_msgs..' ]\n'..M_START..'عدد رسائلك : [ '..user_info_msgs..' ]', 0, 0, 1, nil, dl_cb, nil)
			end
		end
		assert(tdbot_function ({
	...
	if (matches == "id" or matches == "ايدي" or matches == "إيدي") and tonumber(msg.reply_to_message_id) ~= 0 and is_mod(msg) then
		if redis:get(RedisIndex.."lock_cmd"..msg.chat_id) and not is_mod(msg) then return else
		assert(tdbot_function ({
		_ = "getMessage",
		chat_id = msg.chat_id,
