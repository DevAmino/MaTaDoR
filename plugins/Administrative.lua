...
if ((mr_roo[1]:lower() == 'save') or (mr_roo[1] == "حفظ الملف")) and mr_roo[2] then
...
if ((mr_roo[1]:lower() == "sendfile") or (mr_roo[1] == "ارسال ملف")) and mr_roo[2] and mr_roo[3] then
...
if ((mr_roo[1]:lower() == "sendplug") or (mr_roo[1] == "ارسال الملف")) and mr_roo[2] then
...
if ((mr_roo[1]:lower() == 'charge') or (mr_roo[1] == "شحن")) and mr_roo[2] and not mr_roo[3] then
	local linkgp = redis:get(RedisIndex..msg.to.id..'linkgpset')
	if not linkgp then
		return M_START..'`فضلا قبل شحن المجموعه يجب وضع الرابط من خلال الامر التالي`'..EndPm..'\n*"ضع رابط"\n"setlink"*'
	end
	local mods = redis:smembers(RedisIndex..'Mods:'..msg.to.id)
	local owners = redis:smembers(RedisIndex..'Owners:'..msg.to.id)
	if #owners == 0 then
		return M_START..'`فضلا قبل شحن المجموعه قم برفع جميع الادمنيه على البوت من خلال الامر التالي`'..EndPm..'\n*"Config"*\n*"التكوين"*'
	end
	message = '\n'
...
if mr_roo[1]:lower() == 'time sv' or mr_roo[1] == 'وقت التشغيل' then
tdbot.sendMessage(msg.to.id, msg.id, 1, M_START..'`وقت التشغيل :`\n'..os.date("%H:%M:%S")..''..EndPm, 1, 'md')
end
...
if mr_roo[1]:lower() == "monshi" or mr_roo[1] == "منشئ" then
	if mr_roo[2]=="on" or mr_roo[2]=="مفعل" then
	...
	if mr_roo[2]=="off" or mr_roo[2]=="غير مفعل" then
		redis:del(RedisIndex.."bot:pm")
		return M_START.."`منشی غیرفعال شد`" ..EndPm
	end
end
if mr_roo[1]:lower() == "joinch" or mr_roo[1] == "ضع قناه الاشتراك" then
if mr_roo[2] == "on" or mr_roo[2] == "مفعل" then
redis:del(RedisIndex.."JoinEnabel"..msg.chat_id)
return M_START.."`الاشتراك الاجباري مفعل في المجموعه`"..EndPm
elseif mr_roo[2] == "off" or mr_roo[2] == "غير مفعل" then
redis:set(RedisIndex.."JoinEnabel"..msg.chat_id, true)
return M_START.."`الاشتراك الاجباري غير مفعل في المجموعه`"..EndPm
end
end
if (mr_roo[1]:lower() == 'add' or mr_roo[1] == "تفعيل") and not redis:get(RedisIndex..'ExpireDate:'..msg.to.id) then
...
if matches[1]:lower() == "testspeed" or matches[1] == "سرعه السيرفر" then
...
if mr_roo[1]:lower() == 'rem' or mr_roo[1] == "تعطيل" then
...
if mr_roo[1]:lower() == 'gid' or mr_roo[1] == "ايدي المجموعه"  then
	tdbot.sendMessage(msg.to.id, msg.id, 1, '`'..msg.to.id..'`', 1,'md')
end
if mr_roo[1]:lower() == "panelsudo" or mr_roo[1] == "م المطور"  then
	local function inline_query_cb(arg, data)
		if data.results and data.results[0] then
			...
			text = M_START.."هناك خلل في البوت المساعد"..EndPm
...
if (mr_roo[1]:lower() == 'leave' or mr_roo[1] == "مغادره") and mr_roo[2]  then
...
if (mr_roo[1]:lower() == 'charge' or mr_roo[1] == "شحن") and mr_roo[2] and mr_roo[3] then
...
if mr_roo[1]:lower() == 'full' or mr_roo[1] == 'تفعيل غير محدود' then
	local linkgp = redis:get(RedisIndex..msg.to.id..'linkgpset')
	if not linkgp then
		return M_START..'`فضلا قبل تفعيل الوضع الغير محدود قم بوضع الرابط من خلال الامر التالي`'..EndPm..'\n*"ضع رابط"\n"setlink"*'
	end
	local mods = redis:smembers(RedisIndex..'Mods:'..msg.to.id)
	local owners = redis:smembers(RedisIndex..'Owners:'..msg.to.id)
	if #owners == 0 then
		return M_START..'`فضلا قبل شحن المجموعه قم برفع الادمنيه على البوت من خلال الامر التالي`'..EndPm..'\n*"Config"*\n*"التكوين"*'
	end
	message = '\n'
	for k,v in pairs(owners) do
	local user_name = redis:get(RedisIndex..'user_name:'..v) or "---"
	...
	if #mods == 0 then
		return M_START..'`فضلا قبل شحن المجموعه قم برفع الادمنيه من خلال الامر التالي`'..EndPm..'\n*"Config"*\n*"التكوين"*'
	end
	message2 = '\n'
	for k,v in pairs(mods) do
	local user_name = redis:get(RedisIndex..'user_name:'..v) or "---"
...
if (mr_roo[1]:lower() == 'jointo' or mr_roo[1] == "الدخول الى") and mr_roo[2] then
	if string.match(mr_roo[2], '^-%d+$') then
...
if mr_roo[1]:lower() == 'setbotname' or mr_roo[1] == "تغير اسم البوت" then
	tdbot.changeName(mr_roo[2], dl_cb, nil)
	return M_START..'`اسم ربات تغییر کرد به :`\n*'..mr_roo[2]..'*'..EndPm
end
if mr_roo[1]:lower() == 'setbotusername' or mr_roo[1] == "تغير معرف البوت" then
	tdbot.changeUsername(mr_roo[2], dl_cb, nil)
	return M_START..'`یوزرنیم ربات تغییر کرد به :` \n@'..check_markdown(mr_roo[2])..''..EndPm
end
if mr_roo[1]:lower() == 'delbotusername' or mr_roo[1] == "مسح معرف البوت" then
	tdbot.changeUsername('', dl_cb, nil)
	return M_START..'*انجام شد*'..EndPm
end
if mr_roo[1]:lower() == 'markread' or mr_roo[1] == "الخيار الثاني" then
if mr_roo[2]:lower() == 'on' or mr_roo[2] == "مفعل" then
	redis:set(RedisIndex..'markread','on')
	return M_START..'`تیک دوم` *روشن*'..EndPm
end
if mr_roo[2]:lower() == 'off' or mr_roo[2] == "غير مفعل" then
	redis:del(RedisIndex..'markread')
	return M_START..'`تیک دوم` *خاموش*'..EndPm
end
end
if mr_roo[1]:lower() == 'broadcast' or mr_roo[1] == "اذاعه عام" then
...
if mr_roo[1]:lower() == 'sudolist' or mr_roo[1] == "المطورين" then
	return sudolist(msg)
end
if mr_roo[1]:lower() == 'codegift' or mr_roo[1] == 'رقم الهديه' then
...
if mr_roo[1]:lower() == 'giftlist' or mr_roo[1] == 'الهدايا' then
...
if mr_roo[1] == "تنزيل الرتبه" or mr_roo[1]:lower() == "remrank" then
	if msg.reply_id then
		assert (tdbot_function ({
		_ = "getMessage",
		chat_id = msg.to.id,
...
if mr_roo[1]:lower() == "config" or mr_roo[1] == "التكوين" then
	return set_config(msg)
end
if mr_roo[1]:lower() == 'creategroup' or mr_roo[1] == "اصنع مجموعه" then
	local text = mr_roo[2]
	tdbot.createNewGroupChat({[0] = msg.from.id}, text, dl_cb, nil)
	return M_START..'`گروه ساخته شد`'..EndPm
end
if mr_roo[1]:lower() == 'createsuper' or mr_roo[1] == "اصنع مجموعه خارقه" then
local text = mr_roo[2]
tdbot.createNewChannelChat(text, 1, '@batrx', (function(b, d) tdbot.addChatMember(d.id, msg.from.id, 0, dl_cb, nil) end), nil)
	return M_START..'*سوپرگروه ساخته شد و* [`'..msg.from.id..'`] *به گروه اضافه شد.*'..EndPm
end
if mr_roo[1]:lower() == 'tosuper' or mr_roo[1] == "تحويل الى مجموعه خارقه" then
	local id = msg.to.id
...
if mr_roo[1]:lower() == 'bc' or mr_roo[1] == "اذاعه" then
	local text = mr_roo[2]
	tdbot.sendMessage(mr_roo[3], "", 0, text, 0,  "html")
end
if mr_roo[1]:lower() == 'chats' or mr_roo[1] == "المجموعات" then
	return chat_list(msg)
end
if (mr_roo[1]:lower() == 'join' or mr_roo[1] == "دخول") and mr_roo[2] then
...
if (mr_roo[1]:lower() == 'rem' or mr_roo[1] == "تعطيل") and mr_roo[2] then
	...
	return M_START..'*المجموعه* `'..mr_roo[2]..'` *تم تنظيف المدراء من البوت.*'..EndPm
end
if mr_roo[1]:lower() == 'adminlist' or mr_roo[1] == "الادمنيه" then
	return adminlist(msg)
end
if (mr_roo[1]:lower() == 'leave' or mr_roo[1] == "مغادره") and not mr_roo[2] then
	tdbot.sendMessage(msg.to.id, msg.id, 1, M_START..'`تم مغادره البوت`'..EndPm, 1,'md')
	tdbot.changeChatMemberStatus(msg.to.id, our_id, 'Left', dl_cb, nil)
end
if mr_roo[1]:lower() == 'autoleave' or mr_roo[1] == "مغادره تلقائيه" then
	local hash = 'auto_leave_bot'
	if mr_roo[2]:lower() == 'enable' or mr_roo[2] == "مفعل" then
		redis:del(RedisIndex..hash)
		return M_START..'*المغادره التلقائيه مفعله*'..EndPm
	elseif mr_roo[2]:lower() == 'disable' or mr_roo[2] == "غير مفعل" then
		redis:set(RedisIndex..hash, true)
		return M_START..'*المغادره التلقائيه غير مفعله*'..EndPm
	end
end
if mr_roo[1]:lower() == 'panelgp' or mr_roo[1] == 'اعدادات المجموعه' then
	local function inline_query_cb(arg, data)
		if data.results and data.results[0] then
			redis:setex(RedisIndex.."ReqMenu:" .. msg.to.id .. ":" .. msg.from.id, 260, true) redis:setex(RedisIndex.."ReqMenu:" .. msg.to.id, 10, true) tdbot.sendInlineQueryResultMessage(msg.to.id, msg.id, 0, 1, data.inline_query_id, data.results[0].id, dl_cb, nil)
		else
...
if is_mod(msg) then
if msg.to.type == 'channel' or msg.to.type == 'chat' then
if (mr_roo[1]:lower() == 'expire' or mr_roo[1] == "مده التفعيل") and not mr_roo[2] then
	...
	if check_time == -1 then
		remained_expire = M_START..'`مدة التفعيل غير محدودة`'..EndPm
	elseif tonumber(check_time) > 1 and check_time < 60 then
		remained_expire = M_START..'`مدة تفعيل المجموعه` *'..sec..'* `ثانیه`'..EndPm
	elseif tonumber(check_time) > 60 and check_time < 3600 then
		remained_expire = M_START..'`مدة تفعيل الكروب` *'..min..'* `دقية و` *'..sec..'* _ثانیه`'..EndPm
	elseif tonumber(check_time) > 3600 and tonumber(check_time) < 86400 then
		remained_expire = M_START..'`مدة تفعيل المجموعه` *'..hours..'* `ساعه و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه`'..EndPm
	elseif tonumber(check_time) > 86400 and tonumber(check_time) < 2592000 then
		remained_expire = M_START..'`مدة تفعيل المجموعه` *'..day..'* `يوم و` *'..hours..'* `ساعة و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه شحن `'..EndPm
	elseif tonumber(check_time) > 2592000 and tonumber(check_time) < 31536000 then
		remained_expire = M_START..'`مده التفعيل` *'..month..'* `شهر` *'..day..'* `يوم و` *'..hours..'* `ساعة و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه`'..EndPm
	elseif tonumber(check_time) > 31536000 then
		remained_expire = M_START..'`مده التفعيل` *'..year..'* `سنه` *'..month..'* `شهر` *'..day..'* `يوم و` *'..hours..'* `ساعة و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه`'..EndPm
	end
	tdbot.sendMessage(msg.to.id, msg.id, 1, remained_expire, 1, 'md')
end
end
if (mr_roo[1]:lower() == 'expire' or mr_roo[1] == "مده التفعيل") and mr_roo[2]  then
	if string.match(mr_roo[2], '^-%d+$') then
		...
		sec = math.floor(bhours % 60)
		if check_time == -1 then
			remained_expire = M_START..'`مده التفعيل غير محدوده !`'..EndPm
		elseif tonumber(check_time) > 1 and check_time < 60 then
			remained_expire = M_START..'`مدة التفعيل` *'..sec..'* `ثانیه`'..EndPm
		elseif tonumber(check_time) > 60 and check_time < 3600 then
			remained_expire = M_START..'`مده التفعيل` *'..min..'* `دقیقه و` *'..sec..'_ثانيه`'..EndPm
		elseif tonumber(check_time) > 3600 and tonumber(check_time) < 86400 then
			remained_expire = M_START..'`مده التفعيل` *'..hours..'* `ساعة و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه`'..EndPm
		elseif tonumber(check_time) > 86400 and tonumber(check_time) < 2592000 then
			remained_expire = M_START..'`مده التفعيل` *'..day..'* `يوم و` *'..hours..'* `ساعه و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه`'..EndPm
		elseif tonumber(check_time) > 2592000 and tonumber(check_time) < 31536000 then
			remained_expire = M_START..'`مده التفعيل` *'..month..'* `شهر` *'..day..'* `يوم و` *'..hours..'* `ساعه و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه`'..EndPm
		elseif tonumber(check_time) > 31536000 then
			remained_expire = M_START..'`مده التفعيل` *'..year..'* `سنه` *'..month..'* `شهر` *'..day..'* `يوم و` *'..hours..'* `ساعه و` *'..min..'* `دقیقه و` *'..sec..'* `ثانیه`'..EndPm
		end
		tdbot.sendMessage(msg.to.id, msg.id, 1, remained_expire, 1, 'md')
	end
end
if mr_roo[1]:lower() == 'gift' or mr_roo[1] == 'ربح هديه' then 
	redis:setex(RedisIndex.."Codegift:" .. msg.to.id , 260, true)
	tdbot.sendMessage(msg.chat_id, msg.id, 1, M_START.."لديك دقيقتان لإستخدام رقم الهديه مع الوقت الذي لديك"..EndPm, 1, 'md')
end
end
end
end
